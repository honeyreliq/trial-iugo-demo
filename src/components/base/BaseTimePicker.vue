<template>
  <div>
    ...
    <v-row align="center" class="mb-0" no-gutters>
      <fa-icon :icon="['fal', 'clock']" class="fa-fw mr-2" />
      <v-col cols="auto">
        <v-menu bottom :disabled="!showList">
          ...
          <v-list dense height="250" class="ma-0 pa-0">
            <v-list-item-group color="white">
              <v-list-item
                class="white"
                :class="{ primary: item === hour }"
                v-for="(item, i) in getHourList()"
                :key="`hour-list-${i}`"
              >
                <v-list-item-content @click="selectedTimeHandler('hour', item)">
                  {{ item }}
                </v-list-item-content>
              </v-list-item>
            </v-list-item-group>
        ...
        <v-menu bottom :disabled="!showList">
          ...
          <v-list dense height="250" class="ma-0 pa-0">
            <v-list-item-group color="white">
              <v-list-item
                class="white"
                :class="{ primary: item === minute }"
                v-for="(item, i) in getMinuteList()"
                :key="`minute-list-${i}`"
              >
                <v-list-item-content
      ...
      <v-col cols="auto" v-if="format === '12'">
        <BaseButtonGroup :value="time.amPm" class="ml-2" :dense="dense">
          <BaseButton
            v-for="(item, i) in ['AM', 'PM']"
            :key="`am-pm${i}`"
            :value="item"
            :x-small="dense"
    ...
    <v-messages
      v-if="hideDetails === 'auto' ? errors.length > 0 : !hideDetails"
      :value="errors"
      color="error"
      class="mt-3"
    >
...
<script lang="ts">
import { isInteger, isBetween } from '../../common/utils/validation';
...
export default Vue.extend({
  ...
  data() {
    return {
      ...
      minuteRules:
        !this.hideDetails || this.hideDetails === 'auto'
          ? [isInteger, isBetween(0, 59)]
          : [],
      errors: [],
    };
  },
  ...
  methods: {
    ...
    getHourRules(): any[] {
      const hourRules: any =
        this.format === '24' ? isBetween(0, 23) : isBetween(1, 12);
    ...
    hourKeyHandler(event: KeyboardEvent, key: string): void {
      ...
      if (value.length === 2 && value[0] !== '0') {
        const ref: any =
          key === 'hour' ? this.$refs.minuteField : this.$refs.hourField;
        if (!ref.value) {
          ref.focus();
        }
      }
...
::v-deep .time-inputs .v-input__slot {
  min-height: initial !important;
}

::v-deep .hide-spin-arrows {
